...
class ShopifyInstanceEpt(models.Model):
    ...
    def get_shipped_orders(self, record):
        ...
        shipped_query = """select so.id from stock_picking sp
                             ...
                             so.shopify_instance_id=%s""" % record.id
    ...
    def get_total_products(self, record):
        ...
        self._cr.execute("""select count(id) as total_count from shopify_product_template_ept where
                        exported_in_shopify = True and shopify_instance_id = %s""" % record.id)
        ...
        action = record.prepare_action(view,
                                       [('exported_in_shopify', '=', True), ('shopify_instance_id', '=', record.id)])
    ...
    def get_customers(self, record):
        ...
        if result:
            for data in result:
                customer_ids.append(data.get('partner_id'))
        view = self.env.ref('shopify_ept.action_shopify_partner_form').sudo().read()[0]
        action = record.prepare_action(view, [('id', 'in', customer_ids), ('active', 'in', [True, False])])
