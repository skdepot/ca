...
class ShopifyPaymentReportEpt(models.Model):
        ...
        for transaction in remaining_transaction_ids:
            ...
            if transaction.transaction_type == 'charge':
                invoice_ids = order_id.invoice_ids.filtered(
                    lambda l: l.state in ('paid', 'open') and l.type == 'out_invoice')
                if not invoice_ids:
                    continue
            if transaction.transaction_type == 'refund':
                invoice_ids = order_id.invoice_ids.filtered(
                    lambda l: l.state in ('paid', 'open') and l.type == 'out_refund')
                if not invoice_ids:
                    continue
            ...
            if transaction.transaction_type == 'charge':
                payment_reference = self.env['account.payment'].search([('invoice_ids', 'in',
                                                                         invoice_ids.ids), (
                                                                            'amount', '=',
                                                                            transaction.amount), (
                                                                            'payment_type', '=',
            ...
            if payment_reference:
                ...
                reference = invoice_ids.number or ''
    ...
    def generate_bank_statement(self):
        ...
        currency_id = journal.currency_id.id or \
                      self.instance_id.company_id.currency_id.id or False
        ...
        for transaction in self.payout_transaction_ids:
            ...
            account_id = False
            if transaction.transaction_type == 'charge':
                invoice_ids = order_id.invoice_ids.filtered(lambda l: l.state in ('paid', 'open') and
                                                                      l.type == 'out_invoice' and l.amount_total == transaction.amount)
                if not invoice_ids:
                    message = "Invoice is not created for " \
                              "order %s in odoo" % (order_id.name or transaction.source_order_id)
                    payout_logline_obj.create({'message': message,
                                               'is_skipped': True,
                                               'instance_id': self.instance_id.id,
                                               'payout_transaction_ref': transaction.transaction_id,
            ...
            if transaction.transaction_type == 'refund':
                invoice_ids = order_id.invoice_ids.filtered(lambda l: l.state in ('paid', 'open') and
                                                                      l.type == 'out_refund' and l.amount_total == -(
                    transaction.amount))
                if not invoice_ids:
                    message = "In shopify payout there is refund, but Refund is not created for " \
                              "order %s in odoo" % (order_id.name or transaction.source_order_id)
                    payout_logline_obj.create({'message': message,
                                               'is_skipped': True,
                                               'instance_id': self.instance_id.id,
    ...
    def process_bank_statement(self):
        ...
        for statement_line in bank_statement.line_ids:
            try:
                mv_list = []
                payment_aml_rec = []
                mv_line_dicts = []
                ...
                if ref:
                    ...
                    if payment_id:
                        payment_aml_rec = payment_id.mapped('move_line_ids').filtered(lambda line:
                                                                                      line.user_type_id.type == "liquidity")
                else:
                    ...
                    for order in statement_line.shopify_order_ids:
                        if statement_line.amount < 0.0:
                            invoices += order.invoice_ids.filtered(
                                lambda record: record.type == 'out_refund' and record.state in ['open', 'paid'])
                        else:
                            invoices += order.invoice_ids.filtered(
                                lambda record: record.type == 'out_invoice' and record.state in ['open', 'paid'])
                    for invoice in invoices:
                        if invoice.state == 'paid':
                            payment_aml_rec = invoice.payment_ids.mapped('move_line_ids').filtered(lambda
                                                                                                       line: line.user_type_id.type
                                                                                                             == "liquidity")
    ...
    def get_payout_report(self, start_date, end_date, instance):
        ...
        log_book_obj = self.env['shopify.transaction.log']
        ...
        try:
            ...
            payout_ids = []
            if len(shop) == 2:
                url = "{0}//{1}:{2}@{3}/{4}".format(shop[0], instance.api_key, instance.password,
                                                    shop[1], shopify_api_url)
            else:
                url = "https://{0}:{1}@{2}/{3}".format(instance.api_key, instance.password,
                                                       shop[0],
                                                       shopify_api_url)
            ...
            log_book_obj.create({'message':message,
                                 'type':'payout_report',
    ...
    def auto_process_bank_statement(self, ctx={}):
        ...
        if shopify_instance_id:
            ...
            # Closed report if State is processed
